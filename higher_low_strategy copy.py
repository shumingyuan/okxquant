...
class HigherLowStrategy(bt.Strategy):
    params = (
        ...
        ('bounce_thresh', 0.01),   # 回调确认阈值(1%)
        ('stop_loss', 0.02),      # 止损比例(2%)
        ('trailing_stop', 0.02),   # 移动止损比例(2%)
    )
    ...
    def reset_trade_vars(self):
        """重置交易相关变量"""
        super().reset_trade_vars()  # 调用父类的重置方法
    ...
    def next(self):
        ...
        if self.datahigh[0] > self.boll.upper[0]:
            ...
            if not self.in_low_search:
                ...
                if self.in_high_search and self.current_high is not None:
                    ...
                    self.log(f'确认高点: {self.confirmed_high["price"]:.4f}')
        ...
        # 检查是否形成有效的更高低点模式
        if (len(self.pivot_points_high) >= 1 and 
            len(self.pivot_points_low) >= 2):
            last_high = self.pivot_points_high[-1]
            last_low = self.pivot_points_low[-1]
            prev_low = self.pivot_points_low[-2]
            
            if (last_high['price'] > self.confirmed_high['price'] if self.confirmed_high else True and
        ...
        # 入场逻辑
        if self.potential_entry and not self.position:
            if self.wait_count > 0:
                self.wait_count -= 1
            elif self.check_bounce():
                size = self.broker.getcash() * 0.95 / self.dataclose[0]
...
def run_combined_backtest():
    ...
    cerebro.addstrategy(HigherLowStrategy,
                       n_period=30,          # 布林带周期
                       std_multiplier=2.0,   # 布林带宽度
                       wait_bars=3,          # 等待确认K线数
                       bounce_thresh=0.01,   # 回调阈值
                       stop_loss=0.02,       # 止损比例
                       trailing_stop=0.02)   # 移动止损比例
...
if __name__ == '__main__':
    ...
    if run_single:
        # 单文件回测
        run_backtest('data\\doge1m\\DOGE-USDT-SWAP_20220627.csv')
    else:
        # 合并数据回测
